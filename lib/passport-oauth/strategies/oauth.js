...
function OAuthStrategy(options, verify) {
  ...
  this._oauth = new OAuth(options.requestTokenURL, options.accessTokenURL,
                          ...
                          "1.0", options.callbackURL || null, options.signatureMethod || "HMAC-SHA1");
...
OAuthStrategy.prototype.requestTokenParams = function(options) {
  if (options && options.extra_params) {
    var params= {};
    for (var i in options.extra_params) {
      params[i] = options.extra_params[i];
    }
    return params;
  }
  return {};
