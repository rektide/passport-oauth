...
vows.describe('OAuth2Strategy').addBatch({
  ...
  'strategy handling a request to be redirected for authorization with extra_params': {
    topic: function() {
      var strategy = new OAuth2Strategy({
          authorizationURL: 'https://www.example.com/oauth2/authorize',
          tokenURL: 'https://www.example.com/oauth2/token',
    ...
    'after augmenting with actions': {
      topic: function(strategy) {
        ...
        process.nextTick(function () {
          strategy.authenticate(req, { extra_params: { prompt: 'mobile' }});
        });
      },
      ...
      'should redirect to user authorization URL' : function(err, req) {
        assert.equal(req.redirectURL, 'https://www.example.com/oauth2/authorize?response_type=code&redirect_uri=https%3A%2F%2Fwww.example.net%2Fauth%2Fexample%2Fcallback&prompt=mobile&client_id=ABC123&type=web_server');
      },
    },
  },
